...
static BUILD_FEATURES: &[u8] = concat!(
    ...
    // Terminated, so `strings` ends the token here instead of running it into
    // whatever the linker happened to place next. TH-iOS read their stamp as
    // "206557db5-localoff", the trailing "off" being the first of this core's
    // own option values. Harmless to read, not harmless to COMPARE: the deploy
    // guard diffs this token between the library in jniLibs and the stripped
    // copy inside the APK, and those two can legitimately lay out neighbouring
    // data differently, so an untermined token could report a mismatch between
...
pub unsafe extern "C" fn retro_get_system_info(info: *mut retro_system_info) {
    ...
    (*info).library_version = c"0.4.0".as_ptr();
...
mod map_tests {
    ...
    fn the_core_states_which_commit_it_came_from() {
        ...
        // Terminated, so the stamp ENDS where it ends. Without this the linker's
        // next bytes join the token: TH-iOS read theirs as "...-localoff", the
        // "off" being this core's own first option value. The deploy guard
        // COMPARES this token between the library in jniLibs and the stripped
        // copy in the APK, and those can lay out neighbouring data differently,
        // so an unterminated stamp can report two identical builds as a
        // mismatch.
        assert_eq!(
            BUILD_FEATURES.last(),
            Some(&0u8),
            "the feature string must be NUL terminated"
        );
        let s = s.trim_end_matches('\0');
        assert!(!s.contains('\0'), "one terminator, at the end, and nowhere else");
